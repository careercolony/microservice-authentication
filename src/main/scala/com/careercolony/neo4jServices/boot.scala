...
import com.careercolony.neo4jServices.routes.AuthService
...
class StartNeo4jServer(implicit val system: ActorSystem,
                       implicit val materializer: ActorMaterializer) extends AuthService {
  

  def startServer(address: String, port: Int) = {
    Http().bindAndHandle(authRoutes, address, port)
  }
}
